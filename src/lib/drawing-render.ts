...
export function renderDrawing(
  ...
  switch (d.shape.kind) {
    ...
    case 'rectangle': {
      ...
      const cx = r.left - root_r.left + r.width / 2;
      const cy = r.top  - root_r.top  + r.height / 2;
    ...
    case 'underline': {
      ...
      const r = range.getBoundingClientRect();
      const root_r = root.getBoundingClientRect();
      const y = r.bottom - root_r.top + PADDING_EM.underline * remPx();
      const node = rc.line(
        r.left  - root_r.left, y,
        r.right - root_r.left, y,
    ...
    case 'strikethrough': {
      ...
      const r = range.getBoundingClientRect();
      const root_r = root.getBoundingClientRect();
      const y = r.top - root_r.top + r.height / 2;
      const node = rc.line(
        r.left  - root_r.left, y,
        r.right - root_r.left, y,
        { ...DEFAULT_ROUGH, stroke: d.shape.color, strokeWidth: d.shape.width },
    ...
    case 'circle-empty': {
      ...
      const root_r = root.getBoundingClientRect();
      const rem = remPx();
      const cx = r.left - root_r.left + d.shape.anchor.xEm * rem;
      const cy = r.top  - root_r.top  + d.shape.anchor.yEm * rem;
    ...
    case 'margin-bar': {
      ...
      const r = block.getBoundingClientRect();
      const root_r = root.getBoundingClientRect();
      const x = r.left - root_r.left - 0.5 * remPx();
      return [rc.line(
        x, r.top - root_r.top,
        x, r.bottom - root_r.top,
        { ...DEFAULT_ROUGH, stroke: d.shape.color, strokeWidth: d.shape.width },
      )];
    }
    ...
    case 'freehand': {
      ...
      const pts = d.shape.points.map(([xEm, yEm]) =>
        [r.left - root_r.left + xEm * rem, r.top - root_r.top + yEm * rem] as [number, number],
      );
    ...
    case 'freehand-legacy': {
      ...
      for (const s of d.shape.strokes) {
        const pts = s.points.map(([px, py]) =>
          [r.left - root_r.left + px * scale, r.top - root_r.top + py * scale] as [number, number],
        );
